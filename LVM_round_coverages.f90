subroutine round_coverages
    ...
    integer :: ig 
    integer :: ic
    real(sp)  :: area_threshold             ! Decimal percent of each grid cell equal to 1 m^2; Assumes grid_a is in m^2
    real(sp)  :: coverage_area_m2 
    real(sp)  :: coverage_area_flr 
    real(sp)  :: dif 
    ...
    do ig=1,ngrid
        area_threshold = 1/grid_a(ig)
        do ic=1,ncov
            if (coverages(ig,ic,2) < area_threshold) then

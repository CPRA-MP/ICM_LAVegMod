subroutine land_change
    ...
    ! Compare Morph water to Veg water and proceed accordingly
    delta_water = water(:,1) - water_from_morph                         ! 

    ! Loop through every grid cell comparing the amount of water and making the needed changes
    do ig = 1, ngrid
        if ABS(delta_water(ig)) < 0.00390625 then                               ! 1) No change; 0.00390625 = (30*30)/(480*480) is the fraction of one morph cell in a veg grid cell
            ! do nothing
        else if delta_water(ig) > 0 then                                        ! 2) Land gain; add new bareground and subtract water 
            bare_new(ig,2) = bare_new(ig,1) + delta_water(ig)
            water(ig,2) = water(ig,1) - delta_water(ig)
        else delta_water(ig) < 0 then                                           ! 3) land loss; add to water, convert bareground to water, then convert vegetation to water
            delta_water(ig) = ABS(delta_water(ig))                              ! redefining delta water so we're adding a positive rather than having to subtract a negative    
            ! calculate land in Veg cell and Morph cell
            total_flotant = ELBA2_Flt(ig,1) + PAHE2_Flt(ig,1) + bare_Flt(ig,1) + dead_Flt(ig,1)                    !
            morph_land = 1 - (water_from_morph(ig) + upland(ig,1) + total_flotant)
            veg_land = 1 - (water(ig,1) + upland(ig,1) + total_flotant)
            ...
            ! print a warning message -- why is morph land less than 0?
                ...
                if bare_old(ig,1) >= delta_water(ig) then                       ! old bareground is large enough to cover the change
                    bare_old(ig,2) = bare_old(ig,1) - delta_water(ig)
                    water(ig,2) = water(ig,1) + delta_water(ig)
                else if bare_old(ig,1)>0 then                                   ! there is old bareground present, but it's not enough to cover the full land loss 
                    delta_water(ig) = delta_water(ig) - bare_old(ig,1)
                ...
                CLMA20(ig,2)= CLMA10(ig,1)* scale_land           ! CLMA10

subroutine land_change
    ...
    do ig = 1, ngrid                                                                                ! Loop through every grid cell comparing the amount of water and making the needed changes
        ...
        if (delta_water > no_change_threshold) then                                                 ! 2) LAND GAIN; delta_water is positive and greater than change threshold
            ...
            veg_land = 1.0 - (coverages(ig,wti) + coverages(ig,nmi) + total_flotant)                ! calculate all modeled (e.g., non NotMod) land area from last ICM-LAVegMod outputs; note if there is land loss, there should be no new bareground, so coverages(ig,bni) is not needed here
            ...
            if (morph_land < 0.0) then
                ...
                do ic=1,ncov
                    if (cov_grp(ic) > 7) then                                                       ! skip coverage groups for water, bareground, not modeled, flotant, swamp forest, and bottomland hardwood

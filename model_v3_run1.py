...
class Params(object):
    ...
    def config(self, argv):
        ...
        # Step 6.4 to 6.6 Set up the dead floating marsh files.
        # Step 6.4: Get the filename template for the dead floating files
        try:
            outputTemplate = None
        ...
        # Step 6.5: Get the years we want output for the floating dead files
        try:
            outputYear = None
        ...
        # Step 6.6: Build the file names for the separate files.
        if outputTemplate != None and outputYear != None:
            for year in outputYear:
                filename = re.sub(r'<YEAR>', self.yearFormatString.format(year), outputTemplate);
                self.configDict['__DeadFloating_'+str(year)] = filename
...
class WetlandMorphModel:
    ...
    def update_patch(self, newLand, spCoverList, spModelList):
        ...
        curLand = 0.0
        for spName, spCover in itertools.filterfalse( lambda kv: kv[0]=='SAV' or kv[0]=='WATER' or kv[0]=='FIBS' or kv[0]=='DEAD_Flt', iter(spModelList.items())):
...
##\brief An event class to write spatial data on the distribution of dead floating marsh.
##\role{Machinery}
##\detail This class will likely be removed in an upcoming version of the model.
#
class DeadFloatingOutputEvent(landscape.WriteASCIIGrid):
    def __init__(self, time=(0,0), name='DeadFloatingOutputEvent', stream=None, model=None, dataFormat='{0}'):
        landscape.WriteASCIIGrid.__init__(self, time=time, name=name, stream=stream, landscape=None, dataFormat=dataFormat)
        self.model = model

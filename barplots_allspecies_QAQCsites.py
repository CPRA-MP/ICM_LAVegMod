...
for qaqc_dir in ['ecoregion_points','transect_points','CRMS_points']:#,'random_points']:
    ...
    for qf in os.listdir():
        if qf.endswith('.csv'):
            ...
            if gridID > 0:
                ...
                # pull filepaths and qaqc naming tags from output csvs - formats are dependent on directory due to different naming conventions
                if qaqc_dir == 'ecoregion_points':
                    qfL = qf.split('.')[0]
                    QAQCid = qfL.split('_')[11]
                    QAQCdesc = '%s_%s' % (qfL.split('_')[12],qfL.split('_')[13])
                elif qaqc_dir == 'transect_points':
                    qfL = '%s.%s' % (qf.split('.')[0],qf.split('.')[1])
                    QAQCid = qfL.split('_')[11]
                    QAQCdesc = '%s_%s' % (qfL.split('_')[12],qfL.split('_')[13])
                elif qaqc_dir == 'CRMS_points':
                    qfL = qf.split('.')[0]
                    QAQCid = qfL.split('_')[11]
                    QAQCdesc = qfL.split('_')[12]
                elif qaqc_dir == 'random_points':
                    qfL = qf.split('.')[0]
                    QAQCid = qfL.split('_')[11]
                    QAQCdesc = '%s_%s' % (qfL.split('_')[12],qfL.split('_')[13])
...
for row in data_in:
    prj = row[0] #project ID, e.g. G028
    s = row[1] #scenario, e.g. 03
    yr = int(row[2]) #year
    cc = row[3] #coverage code, e.g. 'COES' or 'WATER'
    cc = cc.lstrip(' ') 
    cid = int(row[4]) #cell ID
    area = float(row[5]) #coverage value of that type [0-1]
        ...
        for Cl in list(PSEHts[P][S].keys()): # cell ids 
            ...
            try:
                ...
                for r in range(0,len(years)):
                    if alt == 0:
                        vert = 0.95
                        alt = 1 # set so vertical placemet next year is pulled from alt=1
                    else:
                        vert = 0.97
                        alt = 0 # reset so vertical placement next year is pulled from alt=0
                    plt.text(r+0.55,vert,str(int(BBs[r])),fontsize=6)
...
#                    plt.text(r+0.55,0.97,str(int(BBs[r])),fontsize=6) #vertically stagger every other one
        ...
        #        plt.show()
                ...
                print('-   failed to plot %s %s %s' % (S,P,Cl) )
                print('-    %s' % err) 